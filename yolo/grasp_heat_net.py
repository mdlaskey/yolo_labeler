...
class GHNet(object):
    ...
    def build_network(self,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                net = tf.nn.softmax(net,name='s_max')
        return net
    ...
    def loss_layer(self, predicts, classes, scope='loss_layer'):
        with tf.variable_scope(scope):
            predict_classes = tf.reshape(predicts, [self.dist_size_h,self.dist_size_w])
            
            # class_loss
            class_delta = (predict_classes - classes)
            class_loss = tf.reduce_mean(tf.reduce_sum(tf.square(class_delta), axis=[1, 2]), name='class_loss') 
